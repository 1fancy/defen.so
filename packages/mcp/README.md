...
| `scan_repo` | ✅ | ✅ | ✅ | Bring-your-own-repo SAST + secrets scan of a public `github.com/{org}/{repo}`. Probes the default branch for `.env`, `firebase-adminsdk*.json`, `serviceAccountKey.json`, and 13 secret-family patterns. Also runs **static dead-dependency detection** (declared in `package.json`/`composer.json`/`requirements.txt`/`pyproject.toml` but never imported) and **inert-MCP detection** (servers configured in `.mcp.json` but never referenced) — each returned with a concrete fix. No code is executed. |
